...
double lecture1::PriceByCRR(double S0, double U, double D, double R, int N, double K, double (*Payoff)(double z, double K)) {
...
double lecture1::PriceAnalytic(double S0, double U, double D, double R, int N, double K, double (*Payoff)(double z, double K)) {
    double q = lecture1::riskNeutralProb(U, D, R);
    ...
    for (int i=0; i<=N; i++)
    {
        PDF[i] = lecture1::NewtonSymb(N, i) * pow(q, i) * pow(1 - q, N - i); // binomial distribution
        PDF_Sum += PDF[i];
        Sum += PDF[i] * Price[i];
    }
