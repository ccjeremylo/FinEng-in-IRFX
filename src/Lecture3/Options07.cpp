...
double lecture3::EurOption::PriceByCRR(lecture2::BinModel Model) {
...
double lecture3::AmOption::PriceBySnell(lecture2::BinModel Model) {
    double q = Model.RiskNeutralProb();
    std::vector<double> Price(N_+1);
    double contValue;
    for (int i = 0 ; i <= N_; i++) { // payoff
 ...
 // Call
lecture3::Call::Call(double K, int N) : lecture3::EurOption(N), lecture3::AmOption(N), K_(K) {};
...
double lecture3::Call::Payoff(double z) {
 ...
 // Call
lecture3::Put::Put(double K, int N) : lecture3::EurOption(N), lecture3::AmOption(N), K_(K) {};
