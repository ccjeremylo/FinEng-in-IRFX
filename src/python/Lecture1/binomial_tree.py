...
class BinomialTree:
    def __init__(self, S0, sigma, r, K, T, N, style):
        ...
        self._availiable_style = ["CRR", "JR", "v-drift", "geometric-centred-strike", "arithmetic-centred-strike"]
        assert style in self._availiable_style
        self._style = style
        self._N = N
        self._dt = T/N
        self._R = np.exp(r*T/N) - 1.
        self._set_U_D()
    ...
    def _check_tree(self, tolerance=3e-3):
        ...
        LHS = self._a*(u+d) - d*u
        RHS = self._b
        assert abs(LHS-RHS) -0. < tolerance, f"{LHS} != {RHS}"
    ...
    def _CRR(self):
        ...
        """
        B = -(1. +self._b)/self._a
        u = self._quadratic_solver(1., B, 1., True)
        self._U = u -1.
        self._D = 1./(self._U+1.) -1.
        assert abs(u * (self._D+1.) -1.) < 1e-7, f"{1+self._U} * {1+self._D} != 1"
    ...
    def _JR(self):
        ...
        A = 10.
        B = -12.*self._a 
        C = 4.*self._a**2 - 2*self._b
        u = self._quadratic_solver(A, B, C, True)
        self._U = u -1.
        self._D = 3*u -2*self._a -1.
        q = (self._a - u) / (u - self._D -1.)
    ...
    def prob(self, i):
        return fe.L1_NewtonSymb(self._N, i) * self._q**i * (1-self._q)**(self._N-i)
